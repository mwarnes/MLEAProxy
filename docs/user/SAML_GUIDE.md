...
```bash
java -jar mlesproxy-2.0.5.jar --spring.config.additional-location=./saml.properties
```
...
```bash
# Via command line
java -jar mlesproxy-2.0.5.jar --mleaproxy.saml-debug=true
...
java -jar mlesproxy-2.0.5.jar --mleaproxy.saml-debug=true 2>&1 | grep -i certificate
...
# Set default roles
java -jar mlesproxy-2.0.5.jar --mleaproxy.saml-default-roles=user,reader
```
