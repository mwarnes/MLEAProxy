...
check_prerequisites() {
    ...
    JAR_FILE=""
    if [ -f "$PROJECT_ROOT/mlesproxy-2.0.5.jar" ]; then
        JAR_FILE="$PROJECT_ROOT/mlesproxy-2.0.5.jar"
    elif [ -f "$PROJECT_ROOT/target/mlesproxy-2.0.5.jar" ]; then
        JAR_FILE="$PROJECT_ROOT/target/mlesproxy-2.0.5.jar"
    elif [ -f "$PROJECT_ROOT/release/mlesproxy-2.0.5.jar" ]; then
        JAR_FILE="$PROJECT_ROOT/release/mlesproxy-2.0.5.jar"
    fi
    
    if [ -z "$JAR_FILE" ]; then
        echo -e "${RED}✗ JAR file not found${NC}"
        echo "  Expected: mlesproxy-2.0.5.jar"
        exit 2
    fi
