...
public class KerberosKDCServer {
    ...
    public void start() throws Exception {
        ...
        logger.info("  Realm: {}", config.getRealm());
        logger.info("  KDC: {}:{}", config.getKdcHost(), config.getKdcPort());
    ...
    private List<Principal> importFromLdap() {
        logger.info("Importing principals from LDAP (base DN: {})", config.getLdapBaseDn());
    ...
    private void generateKeytabs() throws Exception {
        ...
        try {
            ...
            for (String servicePrincipal : servicePrincipals) {
                logger.info("    - {}@{}", servicePrincipal, config.getRealm());
            }
        } catch (Exception e) {
    ...
    private void printUsageInstructions() {
        ...
        logger.info("  kinit mluser1@{}", config.getRealm());
        logger.info("  (password: password)");
        logger.info("  klist");
        logger.info("");
        logger.info("Available user principals:");
        logger.info("  mluser1@{}   (password: password)", config.getRealm());
        logger.info("  mluser2@{}   (password: password)", config.getRealm());
        logger.info("  mluser3@{}   (password: password)", config.getRealm());
        logger.info("  appreader@{}  (password: password)", config.getRealm());
        logger.info("  appwriter@{}  (password: password)", config.getRealm());
        logger.info("  appadmin@{}   (password: password)", config.getRealm());
        logger.info("");
        logger.info("Service principals:");
        List<String> servicePrincipals = config.getServicePrincipals();
        if (servicePrincipals != null && !servicePrincipals.isEmpty()) {
            for (String sp : servicePrincipals) {
                logger.info("  {}@{}", sp, config.getRealm());
            }
        }
        logger.info("");
