...
public class BindSearchCustomResultProcessor implements IRequestProcessor, IntermediateResponseListener {
    ...
    public void initialize(RequestProcessorProperties cfg) throws Exception {
        ...
        context.getLogger(BindSearchCustomResultProcessor.class).setLevel(Level.valueOf(cfg.getDebugLevel()));
        context.getLogger(SearchResultListener.class).setLevel(Level.valueOf(cfg.getDebugLevel()));
    ...
    public LDAPMessage processBindRequest(int messageID, BindRequestProtocolOp request, List<Control> controls, LDAPConnection ldapConnection, LDAPListenerClientConnection listenerConnection) {
        ...
        if (cfg.getParm1() != null && !cfg.getParm1().isEmpty()) {
            try {
                bindResult = new LDAPResult(messageID, ResultCode.valueOf(Integer.parseInt(cfg.getParm1())));
            } catch (NumberFormatException e) {
                logger.error("Invalid result code parameter: {}", cfg.getParm1(), e);
    ...
    public LDAPMessage processSearchRequest(int messageID, SearchRequestProtocolOp request, List<Control> controls, LDAPConnection ldapConnection, LDAPListenerClientConnection listenerConnection) {
        ...
        if (cfg.getParm1() == null || cfg.getParm1().isEmpty()) {
            try {
                ResultCode resultCode = (cfg.getParm2() != null && !cfg.getParm2().isEmpty()) 
                    ? ResultCode.valueOf(Integer.parseInt(cfg.getParm2()))
                    : ResultCode.SUCCESS;
                searchResult = new SearchResult(new LDAPResult(messageID, resultCode));
            } catch (NumberFormatException e) {
                logger.error("Invalid result code parameter parm2: {}", cfg.getParm2(), e);
...
//            retAttr.add(uuidObj);
            ...
            if ( !cfg.getParm4().isEmpty()) {
                String[] keyValue = cfg.getParm4().split(":");
                Attribute attr = new Attribute(keyValue[0], keyValue[1]);
                retAttr.add(attr);
            }
            if ( !cfg.getParm5().isEmpty()) {
                String[] keyValue = cfg.getParm5().split(":");
                Attribute attr = new Attribute(keyValue[0], keyValue[1]);
                retAttr.add(attr);
            }
            if ( !cfg.getParm6().isEmpty()) {
                String[] keyValue = cfg.getParm6().split(":");
                Attribute attr = new Attribute(keyValue[0], keyValue[1]);
                retAttr.add(attr);
            }
            if ( !cfg.getParm7().isEmpty()) {
                String[] keyValue = cfg.getParm7().split(":");
                Attribute attr = new Attribute(keyValue[0], keyValue[1]);
                retAttr.add(attr);
            }
            if ( !cfg.getParm8().isEmpty()) {
                String[] keyValue = cfg.getParm8().split(":");
                Attribute attr = new Attribute(keyValue[0], keyValue[1]);
                retAttr.add(attr);
            }
            if ( !cfg.getParm9().isEmpty()) {
                String[] keyValue = cfg.getParm9().split(":");
                Attribute attr = new Attribute(keyValue[0], keyValue[1]);
                retAttr.add(attr);
            }
            if ( !cfg.getParm10().isEmpty()) {
                String[] keyValue = cfg.getParm10().split(":");
                Attribute attr = new Attribute(keyValue[0], keyValue[1]);
                retAttr.add(attr);
            }

            // Build and send new Search Result Entry to client
            SearchResultEntry sre = new SearchResultEntry(messageID,cfg.getParm3(),retAttr);
            entries.add(sre);
            SearchResultEntryProtocolOp searchResultEntryProtocolOp = new SearchResultEntryProtocolOp(cfg.getParm3(),retAttr);
            try {
